...
/*   Updated: 2025/04/07 16:22:14 by aelsayed         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	isvalid_syntax(t_shell *vars)
{
	...
	while (tmp)
	{
		...
		if (!is_par(c) && !is_op(c) && !ft_iswhitespace(c) && tmp->next && is_par(n) && *n == '(')
			return (throw_error(OP), FALSE);
		if (!is_par(c) && is_op(c) && tmp->next && is_par(n) && *n == ')')
			return (throw_error(OP), FALSE);
		if (is_par(c) && *c == '(' && tmp->next && is_op(n) && *n != '<')
			return (throw_error(OP), FALSE);
		if (is_par(c) && *c == ')' && tmp->next && !is_op(n) && !is_par(n) && !ft_iswhitespace(n))
